...
for (const file of files) {
	const logicalPath = file.startsWith(SRC_PREFIX) ? file.slice(SRC_PREFIX.length) : file;
	...
	const inShared = logicalPath.startsWith('shared/') && !logicalPath.startsWith('shared/lib/');
	const inFeature = logicalPath.startsWith('features/');
	const featureSlice = inFeature ? logicalPath.split('/')[1] : '';
