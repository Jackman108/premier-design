...
const EstimateModal: FC<EstimateModalProps> = ({ cards, card, onClose }) => {
	...
	const objectTypeItems = useMemo(() => mapCostingCardsToObjectTypeItems(cards), [cards]);
	const objectTypeLabel = useMemo(() => objectTypeTitleBySelectedTab(cards, selectedTab), [cards, selectedTab]);
				...
				<div className={styles.modal_container}>
					...
					<div className={styles.modal_body}>
						<div className={styles.body_control}>
							...
							/>
							<CostInput handleInputChange={handleInputChange} inputValue={inputValue} />
						</div>
						<div className={styles.body_result} aria-live="polite" aria-atomic="true">
							...
							{!isLoading && error && <div className={styles.error_message}>{error}</div>}
							{!isLoading && result > 0 && (
								<div className={styles.calculate_result}>
									<p>Предварительная оценка стоимости ремонта:</p>
									<p>{result} бел.руб.</p>
								</div>
							)}
						</div>
