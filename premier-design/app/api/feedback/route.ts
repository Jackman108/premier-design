...
import {
	...
	withTimeout,
} from '@shared/lib/api/apiRequestRuntime';
...
export async function POST(request: Request) {
	...
	const finish = (
		statusCode: number,
		options: { status?: 'success' | 'error'; timedOut?: boolean } = {},
	) => {
	...
	if (!rate.allowed) {
		...
		return NextResponse.json(
			createApiErrorPayload(correlationId, 'Too many requests. Try again later.'),
			{ status: 429, headers: jsonHeaders(rate.limitHeaders) },
		);
	}
	...
	if (timedCall.error) {
		...
		return NextResponse.json(createApiErrorPayload(correlationId, 'Failed to process feedback.', { code: statusCode }), {
			status: statusCode,
			headers: jsonHeaders(rate.limitHeaders),
		});
	}
	...
	if (timedCall.timedOut || !timedCall.value) {
		...
		return NextResponse.json(createApiErrorPayload(correlationId, 'Feedback processing timed out.', { code: statusCode }), {
			status: statusCode,
			headers: jsonHeaders(rate.limitHeaders),
		});
	}
	...
	switch (response.status) {
		case 'error': {
			...
			finish(statusCode, { status: 'error' });
			return NextResponse.json({ ...response, correlationId }, { status: statusCode, headers: jsonHeaders(rate.limitHeaders) });
		}
		case 'success': {
			finish(200);
			return NextResponse.json({ ...response, correlationId }, { status: 200, headers: jsonHeaders(rate.limitHeaders) });
		}
		default: {
			const statusCode = 500;
			finish(statusCode, { status: 'error' });
