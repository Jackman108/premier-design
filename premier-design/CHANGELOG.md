...
- **Architecture debt reduction (RISK-03):** контракт `CostingCardProps` вынесен в `shared/interface/CostingCard.props.ts`, калькуляторные shared-модули больше не импортируют `@features/coasting/interface/Costing.props`; `maxAllowedCount` уменьшен до `18`.
...
- **Аудит §3.1, §3.5:** маршруты `/portfolio` и `/calculator` (`pages/portfolio.tsx`, `pages/calculator.tsx`), меню и `titlesPage` в `data/data.json`, sitemap и тест API.
