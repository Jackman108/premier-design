...
| RISK-03 | Архитектурный drift через legacy allowlist boundary-check | Medium | Medium | Рост allowlist, просроченные исключения | `expiresOn` для allowlist, ревизия исключений по спринту | В мониторинге |
...
| RISK-07 | Накопление шумовых артефактов в workspace/PR | Medium | Medium | Появление `.next/dev/cache`, test artifacts, временных логов | `check:noise`, `.gitignore`, pre-commit контроль | Снижен |
...
| RISK-10 | Деградация backend SLO из-за внешних интеграций (SMTP/Telegram/API) | Medium | High | Рост таймаутов API, spike retry в feedback-воронке | Timeouts + retry policy + circuit-breaker для интеграций, алерты по latency/error-rate | В мониторинге |
| RISK-11 | Недостаточная наблюдаемость продакшена (ошибки без корреляции) | Medium | High | Ошибки в логах без route/correlation, медленное RCA | Единый error envelope (`route`, `correlationId`, `status`), дашборд 5xx и клиентских JS-ошибок | Запланирован |
...
3. 🔄 Плановое сокращение legacy-исключений запущено: allowlist вынесен в `scripts/architecture-allowlist.json` (`maxAllowedCount`), добавлен отчет `report:architecture-allowlist`; baseline уменьшен на 2 устаревших исключения.
