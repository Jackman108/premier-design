...
| RISK-07 | Накопление шумовых артефактов в workspace/PR                                  | Medium      | Medium  | Появление `.next/`, `src/.next/`, `coverage/`, `src/coverage/`, отчётов e2e/storybook | `check:noise`, `.gitignore` / `.dockerignore` / `.prettierignore` (корень и `src/`), pre-commit контроль                           | Снижен                                                                                                     |
