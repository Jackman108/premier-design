...
2. **UI — тонкий слой.** В `*/ui/*` запрещены бизнес-правила, **реализация** хуков, объявление типов (кроме локальных props компонента), вызовы внешних API/IO и инфраструктуры. Сопутствующее — в `hooks/`, `interface/` (или `types/`), `utils/`, валидаторы в `shared/validates/`, обвязка Next.js в **`shared/lib/`** (`@shared/lib/*`). Контроль: `yarn check:ui-purity`. Подробности: [`02-web-ui-components-and-tokens-ru.md`](02-web-ui-components-and-tokens-ru.md).
