...
- **a11y (минимум как у headless):** скрытые `h2` + `p` с `useId()`, связь через `aria-labelledby` / `aria-describedby`; подписи поверх затемнения — **`var(--white)`** + лёгкий `text-shadow` (читаемость в тёмной теме, см. [`docs/mempalace/rules/02-web-ui-components-and-tokens-ru.md`](../mempalace/rules/02-web-ui-components-and-tokens-ru.md) и [`03-web-a11y-and-ux-ru.md`](../mempalace/rules/03-web-a11y-and-ux-ru.md)).
