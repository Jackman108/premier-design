...
- В CI включен quality-gate `yarn check:perf:ci` (Lighthouse mobile + budget initial JS после `yarn build`). **`check:perf:initial-js`** суммирует JS по `build-manifest.json`: для App Router — оболочка **`polyfillFiles` + `rootMainFiles`** (если нет legacy-чанков для `/`). Пороговые env: `PERF_BUDGET_SCORE`, `PERF_BUDGET_LCP_MS`, `PERF_BUDGET_CLS` (по умолчанию **0.17** в скрипте и в `ci.yml`: lab mobile часто выше полевых целей), `PERF_BUDGET_INP_MS`, `PERF_BUDGET_TBT_MS`, `INITIAL_JS_BUDGET_KB`.
...
- **Windows:** по умолчанию шаг Lighthouse в `check:perf:lighthouse` пропускается (нестабильный headless / interstitial). В консоль всё равно выводятся **пороги CI**; фактические цифры — в логе Linux job или в файле `.audit/lighthouse-perf-summary.json` в корне репозитория после прогона (на Windows при пропуске там `skipped: true`). Полный gate в CI (GitHub Actions). Локально принудительно: `PERF_AUDIT_FORCE_LIGHTHOUSE=true`. Отключить Lighthouse: `PERF_AUDIT_SKIP_LIGHTHOUSE=true`.
