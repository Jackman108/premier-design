...
- **Windows:** по умолчанию шаг Lighthouse в `check:perf:lighthouse` пропускается (нестабильный headless / interstitial). В консоль всё равно выводятся **пороги CI**; фактические цифры — в логе Linux job или в файле `premier-design/.audit/lighthouse-perf-summary.json` после прогона (на Windows при пропуске там `skipped: true`). Полный gate в CI (GitHub Actions). Локально принудительно: `PERF_AUDIT_FORCE_LIGHTHOUSE=true`. Отключить Lighthouse: `PERF_AUDIT_SKIP_LIGHTHOUSE=true`.
