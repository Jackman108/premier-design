...
- Каталог **`lib/`** (`@lib/*`): обвязка под Next.js без смешения с `shared/` — например **`getStaticData.ts`** → **`loadSiteData()`** из **`@shared/site-data`** (контент **`data/locales/ru/data.json`**, **`dataPropsSchema`**; PD-R-06, только RU). Дев-снимок заявок — **`formData.json`**, **`formDataPersistenceSchema`**. Далее **`dynamicSectionImports.ts`**, **`staticPropsHandler.ts`**, **`shared/utils/staticPathsHandler`**, **`findRelatedService`**, **`collectSitePathnames.ts`**, **`resolveServicesTier.ts`** + **`servicesTierStatic.ts`**. Тип корня **`DataProps`** = **`z.infer<typeof dataPropsSchema>`**; ошибки формы — **`feedbackFormValidationMessages`**. `widgets/interface/interfaceData.ts` реэкспортирует **`DataProps`** и производные алиасы (`GetDataProps`, `FaqContentByPage`, …).
