...
**Тесты хуков:** `**/hooks/__tests__/*.test.ts(x)` рядом с реализацией. Отдельного корня `hooks/` в репозитории нет; алиас `@hooks/*` не задаётся.
...
- **`pages-layer/`** (`@pages`, `@pages/*`, `@pages-layer/*`): клиентские и смешанные композиции (например главная — **`HomePageClient.tsx`** + **`home-route.tsx`** / **`home-metadata.ts`**); в **`app/*/page.tsx`** — тонкий реэкспорт (паритет Feb Code). Конфиг сегмента (**`revalidate`**, `dynamic`, …) задаётся в **`app/.../page.tsx`**, не реэкспортом из `pages-layer` — иначе Next.js не парсит его на этапе сборки. Алиасы **`@app`**, **`@app/*`** на каталог **`app/`**.
- Каталог **`shared/lib/`** (импорты **`@shared/lib/...`**; бывший корень **`lib/`**, ADR 0013): Next-обвязка, SEO, API-runtime, динамические секции, `getStaticData` — рядом с остальным **`shared/lib`**. Данные сайта: **`loadSiteData(locale?)`** в **`@shared/site-data`** (на клиенте импортируйте типы/константы из **`@shared/site-data/site-locale`** и т.п., не из барреля **`@shared/site-data`**, если модуль попадает в клиентский бандл — иначе подтянется **`resolve-request-site-locale`** / **`next/headers`**); на запрос используйте **`getCachedSiteBundle()`** из **`@shared/lib/getStaticData`** (одно чтение cookie + бандл), не пара **`getCachedData` + `resolveRequestSiteLocale`**. Канон продакшена — **`ru`**; **`en`** — полный бандл **`data.json`** / **`ui.json`** (базовая вычитка ~~**DATA-PREM-01**~~). UI-переводы (не доменный JSON): **`data/locales/<locale>/ui.json`**, контекст **`LocaleProvider`** / **`useLocale`** в **`@shared/i18n`** (подход как во Feb Code). SEO: **`proxy.ts`** → заголовок **`x-pathname`**, **`hreflangAlternatesForPath`** в корневом **`generateMetadata`**. Дев-**`formData.json`** — **`formDataPersistenceSchema`**. Далее **`dynamicSectionImports`**, **`staticPropsHandler`**, **`shared/utils/staticPathsHandler`**, **`findRelatedService`**, **`collectSitePathnames`**, **`resolveServicesTier`**, **`servicesTierStatic`**, **`query/react-query-config`**. Тип корня **`DataProps`**; ошибки формы — **`feedbackFormValidationMessages`**. `widgets/interface/interfaceData.ts` — реэкспорты.
- **Константы вне контентного JSON:** `shared/constants/company.ts` — единый источник: **`SITE_OPERATOR`** (production-origin `publicOrigin`, бренд, год с копирайта, e-mail для ПДн, краткое юр. наименование, блок для JSON-LD: соцсети, адрес, `areaServed`, `priceRange`), **`SITE_PUBLIC_ORIGIN`** (алиас origin для sitemap/canonical), подрядчик **`DEVELOPER_STUDIO_FEB_CODE`**. Юридические страницы и SEO импортируют отсюда; `public/robots.txt` (строка Sitemap) вручную синхронизировать с `SITE_OPERATOR.publicOrigin`.
...
- Крупные view-файлы в `features/` / `widgets/` / `pages-layer/` не превышают порога декомпозиции без записи в **`scripts/decomposition-allowlist.json`** с **`expiresOn`** и **`owner`** (см. **`yarn report:decomposition-threshold`**).
