...
const ActionProvider: FC<ActionProviderProps> = ({ createChatBotMessage, setState, children }) => {
	...
	const handleServices = () =>
		sendMessage(
			...
			setState,
			'Мы предоставляем широкий спектр услуг по ремонту интерьеров, включая покраску стен, укладку полов, установку мебели и многое другое. Расскажите нам, что вас интересует, и мы с радостью поможем вам с вашим проектом!',
		);
	const handlePortfolio = () =>
		sendMessage(
			createChatBotMessage,
			setState,
			'У нас есть много успешных проектов в нашем портфолио.Можете посмотреть его на нашем сайте в разделе "Примеры работ".',
		);
	const handlePricing = () =>
		sendMessage(
			createChatBotMessage,
			setState,
			'Для получения информации о ценах на наши услуги, пожалуйста, свяжитесь с нами по указанным контактам.',
		);
	const handleAppointment = () =>
		sendMessage(
			createChatBotMessage,
			setState,
			'Для записи на прием, пожалуйста, свяжитесь с нами по указанным контактам или заполните форму на нашем сайте.',
		);
