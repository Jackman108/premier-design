...
describe('submitFeedback', () => {
	...
	it('saves to file, sends email and telegram in development mode', async () => {
		...
		expect(mailTransport).toEqual(expect.objectContaining({ connectionTimeout: 5_000, socketTimeout: 6_000 }));
	...
	it('retries SMTP send once on transient error and succeeds', async () => {
		mockedEmailService.sendMail.mockRejectedValueOnce(new Error('SMTP ETIMEDOUT')).mockResolvedValueOnce(undefined);
